...
def generate_time_series(start: datetime, minutes: int):
    """Generate a list of timestamps, one per simulated minute."""
    return [start + timedelta(minutes=i) for i in range(minutes)]

def run_simulation_cli(cfg, max_pods=5):
    """Run the full simulation and return list of rows."""
    generator = DataGenerator(max_pods=max_pods, pod_scale_threshold=getattr(cfg, "pod_scale_threshold", 50))
    rows = []
